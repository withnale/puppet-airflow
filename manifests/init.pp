...
class airflow (
  ...
  $service_name            = $::airflow::params::service_name,
  $subpackages             = $::airflow::params::subpackages,

  $git_repo                = undef,
  $git_ref                 = master,

  $database                = $::airflow::params::database,
  ...
  $prerequisite_packages   = $airflow::params::prerequisite_packages
  ### END hiera lookups ###
  ...
  if ($install_flower) {
    class { '::airflow::service::flower':
      subscribe => Class['::airflow::config']
    }
  }
